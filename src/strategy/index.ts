...
export const BigMoney: SimFunction = state => {
  const { money } = state;
...
export const SmithyBigMoney: SimFunction = state => {
  const { money } = state;
  const smithy = state.gained.filter(c => c === BaseSet.Smithy).length;
  if (money >= 8) return BaseCards.Province;
  if (money >= 6) return BaseCards.Gold;
  // if less than 2 smithy in deck
  if (money >= 4 && smithy < 10 && smithy < 2) return BaseSet.Smithy;
  // pick up a 3rd smithy after turn 10
  if (money >= 4 && smithy < 10 && smithy < 3 && state.turnNum > 10) return BaseSet.Smithy;
  if (money >= 3) return BaseCards.Silver;
};

export const VillageSmithy: SimFunction = state => {
  const { money } = state;
  const smithy = state.gained.filter(c => c === BaseSet.Smithy).length;
  const village = state.gained.filter(c => c === BaseSet.Village).length;
  if (money >= 8) return BaseCards.Province;
  if (money >= 6) return BaseCards.Gold;
